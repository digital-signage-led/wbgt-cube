 ...
 *
 * まとめ取得（サイネージ推奨）:
 *   <exec>?type=bundle&point=63801&alertArea=兵庫県
 ...
 *
 * 熱中症アラートのみ:
 *   <exec>?type=alert&point=63801&alertArea=兵庫県
 *
 * 大雨警報（気象庁）:
...
function doGet(e) {
  ...
  if (type === 'rainwarn') {
    var area = normalizeWarnArea_(params.area || '270000');
    var city = normalizeWarnCity_(params.city || '2710000');
    ...
    try {
      if (type === 'alert') {
        ...
        payload = buildRainWarnPayload_(
          normalizeWarnArea_(params.area || '270000'),
          normalizeWarnCity_(params.city || '2710000')
        );
      } else if (type === 'bundle') {
...
function normalizeWarnArea_(raw) {
  return String(raw || '').trim().replace(/\D/g, '') || '270000';
}

function normalizeWarnCity_(raw) {
  return String(raw || '').trim().replace(/\D/g, '') || '2710000';
}
...
function tryMoeFallbackPayload_(point, params, headerName) {
  var fb =
    MOE_FALLBACK_POINT[point] ||
    (params.fallbackPoint ? String(params.fallbackPoint).trim() : '');
  if (!fb || fb === point || params._noFallback) return null;
  var p2 = {};
  var k;
  for (k in params) {
    if (Object.prototype.hasOwnProperty.call(params, k)) p2[k] = params[k];
  }
  p2._noFallback = '1';
  var alt = buildWbgtPayload_(fb, p2);
  if (!alt || alt.source !== 'moe-env.go.jp') return null;
  alt.point = String(point);
  alt.pointName = resolvePointName_(point, headerName, params);
  alt.dataVia = 'fallback-' + fb;
  return alt;
}
