/**
 * 環境省 WBGT プロキシ（Web App）— 大阪市用
 *
 * 地点はクエリ ?point= で指定（既定 62078=大阪市）
 * データ源（環境省公式CSV）:
 ...
 * 発行URLを ../index.html の DEFAULT_MOE_GAS_URL に設定
 */
var TZ = 'Asia/Tokyo';

/** @type {Object.<string, {prefKey: string, pointName: string}>} */
var POINT_TABLE = {
  '62078': { prefKey: 'osaka', pointName: '大阪市' }
};

var DEFAULT_POINT = '62078';
...
function test_sendai_() {
  Logger.log(JSON.stringify(buildWbgt_({ parameter: { point: '34392' } }), null, 2));
}
