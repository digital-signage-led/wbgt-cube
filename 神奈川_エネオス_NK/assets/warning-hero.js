...
(function (global) {
  ...
  function municipalityLabel_(name, code) {
    ...
    var configured = (cfg && cfg.jma && cfg.jma.warnCityLabel) || '磯子区';
    var warnCity = (cfg && cfg.jma && cfg.jma.warnCity) || '1410012';
    if (c && String(c) === String(warnCity)) return configured;
    if (c === '1410012' || n.indexOf('磯子') >= 0) return configured;
    var city = n.match(/^(.*?市)/);
    if (city && /区/.test(n)) {
      var ku = n.match(/([^市]+区)$/);
      if (ku && ku[1] === '磯子区') return configured;
      return city[1];
    }
    if (/区$/.test(n) && n.indexOf('県') < 0) {
      if (n === '磯子区') return configured;
      return n;
    }
    if (/[市町村]$/.test(n) && n.indexOf('県') < 0) return n;
    if (c === '1410012') return configured;
    return configured;
  }
  ...
  function liveMatchCodes(cityCode, areaCode) {
    ...
    var area = String(areaCode || '');
    if (area && codes.indexOf(area) < 0) codes.push(area);
    return codes;
  }
