...
- マーカー: `<!-- clock: jst-network-v1 -->`
- `CLOCK_TZ = 'Asia/Tokyo'` / `jstPartsFromMs` / `hourCycle: 'h23'`
- `TIME_SYNC_CHAIN`: Cloudflare → timeapi.io → worldtimeapi → NICT
- `parseTimeApiIoJson_`: オフセット無しは `+09:00`
- デモ（`DEMO_SIGNAGE_ACTIVE`）でも起動時＋60秒周期で `syncTimeFromNetwork`
- 同期全失敗かつ未同期時のみ `Date.now()` → 表示は Asia/Tokyo
